...
module {
	toy.func @reduction_performed() {
		...
		%1 = toy.reduce_max %0 {axis = 1 : i64} : tensor<2x2xf64> -> tensor<2xf64>
...
// REDUCE: [[CST:%.*]] = toy.constant dense<[5.000000e+00, 3.000000e+00]> : tensor<2xf64>
// REDUCE-NOT: toy.reduce_max
...
module {
	toy.func @rank_zero() {
		...
		%1 = toy.reduce_max %0 {axis = 0 : i64} : tensor<f64> -> tensor<f64>
		toy.print %1 : tensor<f64>
		toy.return
	}
}

// RANK-ZERO: error: 'toy.reduce_max' op reduce_max expects a tensor with rank > 0.
...
module {
	toy.func @axis_oob() {
		...
		%1 = toy.reduce_max %0 {axis = 2 : i64} : tensor<2x2xf64> -> tensor<2xf64>
		toy.print %1 : tensor<2xf64>
		toy.return
	}
}

// AXIS-OOB: error: 'toy.reduce_max' op axis 2{{.*}}out of bounds for tensor of rank{{.*}}2
...
// AXIS-NEGATIVE: error: 'toy.reduce_max' op axis -1{{.*}}out of bounds for tensor of rank{{.*}}2
...
module {
	toy.func @result_shape_mismatch() {
		%0 = toy.constant dense<[[1.000000e+00, 5.000000e+00], [3.000000e+00, 2.000000e+00]]> : tensor<2x2xf64>
		%1 = toy.reduce_max %0 {axis = 1 : i64} : tensor<2x2xf64> -> tensor<3xf64>
		toy.print %1 : tensor<3xf64>
		toy.return
	}
}

// RESULT-SHAPE: error: 'toy.reduce_max' op has incorrect result type: expected 'tensor<2xf64>' but got 'tensor<3xf64>'
