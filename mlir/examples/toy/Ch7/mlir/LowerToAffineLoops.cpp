...
// Miror the BinaryOP lowering above for unary ops.
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    lowerOpToLoops(op, operands, rewriter,
                   [loc](OpBuilder &builder, ValueRange memRefOperands,
                     ...
                     // Generate a load for the element of 'input' at the
                     // inner loop.
                     auto loadedInput = builder.create<affine::AffineLoadOp>(
                         loc, unaryAdaptor.getInput(), loopIvs);

                     // Create the unary operation performed on the loaded value.
